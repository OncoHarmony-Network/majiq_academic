...
def rank_majiq(vlsv_list, V=0.2, absolute=True, dofilter=True, E=False, ranknochange=False, complex_lsvs=False,
    ...
    for i, vlsv in enumerate(vlsv_list):
        ...
        if len(lsv_bins) > 2:
            if junc_selection:
                bins_selected = lsv_bins[junc_selection[vlsv.lsv_graphic.gene_id]]
                junc_n = junc_selection[vlsv.lsv_graphic.gene_id]
            else:
                most_change = 0
...
def main():
	...
	from voila.io_voila import VoilaInput 
        ...
        for file_nr, file_str in enumerate(args.majiq_files):
            majiq_data = VoilaInput.from_hdf5_file(hdf5_filename=file_str)
		...
		print "MAJIQ 1st delta(psi) AFTER RANK pair %d" % len(majiq_file1_names)
            ...
            if args.type_rank == 'only_exp1':
                ...
                exp1_index = np.array([v_lsv.lsv_graphic.gene_id in majiq_file1_names for v_lsv in majiq_data.get_lsvs()])
                junc_dict = dict(

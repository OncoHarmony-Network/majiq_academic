...
def calc_psi(alpha, n, debug, *samples_events):
    ...
    psi_matrix = []
    for event_num, event_samples in enumerate(range(samples_events[0].shape[
        0])):  # we iterate through the second dimension of the matrix, which corresponds to the paired samples per event for different experiments
        if event_num % 50 == 0:
...
def main():
    ...
    parser.add_argument('--debug', type=int, default=0,
                        help="Activate this flag for debugging purposes, activates logger and jumps some processing steps.")
    parser.add_argument('--minreads', default=10, type=int,
                        help='Minimum number of reads combining all positions in an event to be considered. [Default: %(default)s]')
    ...
    parser.add_argument('--markstacks', default=0.0000001, type=float,
                        help='Mark stack positions. Expects a p-value. Use a negative value in order to disable it. [Default: %(default)s]')
    parser.add_argument('--tracklist', nargs='+',
                        help='A list of identifiers to track in detail, for debugging purposes')
    ...
    parser.add_argument('--files-contain-scores', dest="files_contain_scores", action='store_true', default=False,
                        help='Feed the script with psis already computed.')

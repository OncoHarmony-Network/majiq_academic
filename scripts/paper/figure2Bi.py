...
def add_method(method_name, method_num, targetdir, group_folder):
    ...
    for i, ratio_path in enumerate(ratios):
        ...
        if group_folder in rep_names[i]:
            ...
            plt.plot(np.linspace(0, 1, len(ratio)), ratio, '-',
                     label="%s Groups - %.0f%% (N=%d)" % (method_name.upper(), math.ceil(ratio[-1]*100), numevents),
                     linewidth=4, color=rgb_to_hex(cb.Paired[10][2*method_num + 1]), alpha=1)
            continue
    ...
    plt.plot(np.arange(0+1./(2*bins_area), 1, 1./bins_area), np.mean(ndbins, axis=0),
             label="%s Pairs mean - %.0f%% (Avg. N=%.0f)" % (method_name.upper(),
                                                             math.ceil(np.mean(ndbins, axis=0)[-1]*100),
                                                             np.mean(n_list)),
             color=rgb_to_hex(cb.Paired[10][2*method_num + 1]), lw=2)
    # plt.plot(np.arange(0, 1, 1./nbins_n1.shape[1]), np.mean(nbins_n1, axis=0), '--',
    # label="%s Pairs Null Model & STD - %.0f%%" % (method_name.upper(),
    # math.ceil(np.mean(nbins_n1, axis=0)[-1]*100)), color=rgb_to_hex(cb.Paired[10][2*method_num + 1]),
    # lw=2)

    plt.fill_between(np.arange(0+1./(2*bins_area), 1, 1./bins_area),
                     np.mean(ndbins, axis=0)-np.std(ndbins, axis=0),
                     np.mean(ndbins, axis=0)+np.std(ndbins, axis=0),
                     facecolor=rgb_to_hex(cb.Paired[10][2*method_num]), lw=0, alpha=alpha_plt)
...
def main():
    ...
    parser.add_argument('targetdir', type=str,
                        help='Folder containing the ratios (Pickle files) for each method.')
    parser.add_argument('--plotpath', required=True,
                        help='Output file path.')
    parser.add_argument('--plotname', default='figure2b',
                        help='Filename of the plot.')
    parser.add_argument('--plottitle', default='dPSI Reproducibility - H124L123 Vs H56L45',
                        help='Title of the plot.')
    parser.add_argument('--groupfolder', default='H124L123H56L45',
                        help='Group subfolder name with the ratios for delta psi groups. '
                             '[Only if there are group comparisons].')
    parser.add_argument('--extension', choices=('pdf', 'png'), default='pdf',
                        help='Format of the plot generated.')
    parser.add_argument('--methods', dest='methods_set', nargs='*', default=methods,
                        help='Methods to be compared. It could be any combination '
                             'of: %s.' % ', '.join(methods))

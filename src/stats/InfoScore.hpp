...
#ifndef MAJIQSTATS_INFOSCORE_HPP
#define MAJIQSTATS_INFOSCORE_HPP
...
pybind11::array_t<double> InfoScore(
  ...
  for (pybind11::ssize_t i = 0; i < _x.shape(0); ++i) {
    ...
    const pybind11::ssize_t n = static_cast<pybind11::ssize_t>(n_neg + n_pos);
    double min_score = std::numeric_limits<double>::max();
    int offset = 0;  // initial offset
    for (pybind11::ssize_t k = 0; k < n; ++k) {
      offset += _labels(i, _sortx(i, k)) ? 1 : -1;
      min_score = std::min(
          min_score,
          InfoScorePathScore(n_neg, n_pos, 1 + k, offset));
    }  // loop over offsets to compute minimum score
    // lhs has counts for both groups now, so:
    _result(i) = tester.CalculatePValue(n_neg, n_pos, min_score);
  }
...
#endif  // MAJIQSTATS_INFOSCORE_HPP

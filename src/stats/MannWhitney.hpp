...
#ifndef MAJIQSTATS_MANNWHITNEY_HPP
#define MAJIQSTATS_MANNWHITNEY_HPP
...
pybind11::array_t<double> MannWhitneyU(
  ...
  for (pybind11::ssize_t i = 0; i < _x.shape(0); ++i) {
    // calculate MannWhitneySummary for the row
    MannWhitneySummary summary;
    {
      ...
      pybind11::ssize_t idx = 0;
      int last_rank = 0;
      while (idx < _x.shape(1)) {
        // current value
        const auto& val = _x(i, _sortx(i, idx));
        if (std::isnan(val)) {
        ...
        for (; idx < _x.shape(1) && _x(i, _sortx(i, idx)) == val; ++idx) {
    ...
    _result(i) = tester.CalculatePValue(summary);
  }
...
#endif  // DEFINE MAJIQSTATS_MANNWHITNEY_HPP

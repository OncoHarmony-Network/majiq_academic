    ...
    try:
        ...
        for lidx, lsv_id in enumerate(list_of_lsv):
            ...
            for grp_idx in range(2):
                ...
                num_ways = all_sample.shape[1]
                alpha_prior, beta_prior = get_prior_params(lsv_type, num_ways)
                mu_psi = np.zeros(shape=(num_exp[grp_idx], num_ways))
                mean_psi = np.zeros(shape=(quantification_init.nbins, num_ways), dtype=np.float)
                samps[grp_idx] = np.zeros(shape=(num_ways, quantification_init.nsamples))
                for exp in xrange(num_exp[grp_idx]):
                    for p_idx in xrange(num_ways):
                        ...
                        mean_psi[p_idx] += post_psi / quantification_init.m
                ...
                mean_psi[p_idx] /= num_exp[grp_idx]
                lsv_het.add_group(mu_psi, mean_psi, quantification_init.names[grp_idx])

            out_stats = do_test_stats(samps, quantification_init.stats, quantification_init.minsamps)
            lsv_het.add_junctions(out_stats)

                # if num_ways == 2:
                #     break

            qm = QueueMessage(QUEUE_MESSAGE_HETER_DELTAPSI, [lsv_het], chnk)
        ...
        for jn_idx in range(njuncs):
            ...
            for samp_idx in range(nsamples):
                ...
                for stat_idx, stat_name in enumerate(stats):
                    cur_output[stat_idx, samp_idx] = stat_name.title()(csamps, clabels)
        ...
        if len(list_of_lsv) > 0:
            ...
            pool = mp.Pool(processes=self.nthreads, initializer=quantification_init,
                           initargs=[q, lock_arr, self.output, self.name, self.silent, self.debug, self.nbins, self.m,
                                     self.k, self.discardzeros, self.trimborder, self.files, self.only_boots, None,
                                     file_locks],
                           maxtasksperchild=1)

...
def local_weight_eta_nu_per_lsv(group, nexp):
    ...
    for lidx, lsv in enumerate(group):
        for eidx, exp_lsv in enumerate(lsv):
            ...
            median_psi[lidx, eidx] = np.median(np.array(mpsi[max_eta_idx[lidx, eidx]]), axis=0)
...
def local_weights(replicas, l1=False, median_ref=np.array([])):
    """

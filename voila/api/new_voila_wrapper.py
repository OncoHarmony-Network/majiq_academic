    ...
    with SpliceGraph(get_builder_splicegraph_filename(outDir)) as sg:
        for gid, jlist in dict_junctions.items():
            for xx in jlist.values():
                ...
                sg.junction.update_reads(gene_id=gid, start=xx.start, end=xx.end, experiment=exp, reads=cov)
    lock.release()
    ...
    for gne_id, gne in dict_of_genes.items():
        ...
        with SpliceGraph(get_builder_splicegraph_filename(majiq_config.outDir)) as sg:
            for jid in sorted(dict_junctions[gne_id].keys()):
                ...
                junc_list.append(
                    sg.junction.add(gene_id=gne_id, start=jj.start, end=jj.end, annotated=jj.annot,
            ...
            for ex in sorted(exon_dict[gne_id], key=lambda x: (x.start, x.end)):
                ...
                exon_list.append(
                    sg.exon.add(gene_id=gne_id, start=ex_start, end=ex_end, coords_extra=extra_coords,
                                intron_retention=False, annotated=ex.annot, alt_starts=alt_start, alt_ends=alt_ends)
                )

            for info in list_introns[gne_id]:
                intr_coord = int(info.start) - 1
                junc_list.append(
                    sg.junction.add(gene_id=gne_id, start=intr_coord, end=info.start,
                                    intron_retention=voila_const.IR_TYPE_START))

                a3 = [len(junc_list) - 1]
                intr_coord = int(info.end) + 1
                junc_list.append(
                    sg.junction.add(gene_id=gne_id, start=info.end, end=intr_coord,
                                    intron_retention=voila_const.IR_TYPE_END))

                a5 = [len(junc_list) - 1]
                eg = sg.exon.add(gene_id=gne_id, annotated=info.annot,
                                 start=info.start, end=info.end, coords_extra=(), intron_retention=True)

                exon_list.append(eg)

            sg.gene.add(gne_id, name=gne['name'], strand=gne['strand'], exons=exon_list,

...
def generate_lsv(i, lsvs_bins, confidence, **kwargs):
    ...
    if 'names' in kwargs and kwargs['names']:
        id = kwargs['names'][i]
        try:
        ...
        matrix_link = "#"
        if 'keys_plots' in kwargs:
            # Correct key names for plots
            for key_plot, name_plot in kwargs['keys_plots'].items():
    ...
    lsv_type = ''
    if 'lsv_type' in kwargs:
        lsv_type = kwargs['lsv_type']

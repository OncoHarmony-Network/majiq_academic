    ...
    render_lsv_splice_graph(canvas) {
        this.db_gene.get(canvas.getAttribute('data-gene-id')).then((gene) => {
            if (canvas.getContext) {
                ...
                let exon_lsv_number = '';
                let exon_lsv_coords = canvas.getAttribute('data-coord-exon');
                const exons_gene = gene.exons.filter(function (d) {
                ...
                if (exon_lsv_coords) {
                    ...
                    const lsv_exon = exons_gene.find(function (element) {

...
def table_marks_set(size):
    ...
    for index in range(len(ideal_set)):
    ...
    if type_summary == constants.ANALYSIS_PSI:
        ...
        while count_pages * constants.MAX_GENES < len(gene_keys):
            ...
            with open(full_path, 'w') as voila_output:
                table_marks = [table_marks_set(len(gene_set)) for gene_set in genes_dict]
                st = sum_template.render(tableMarks=table_marks, genes_dict=genes_dict, prevPage=prev_page,
                                         nextPage=next_page, namePage=name_page, lexps=majiq_output['meta_exps'],
                                         genes_exps_list=majiq_output['genes_exp'],
                                         comb_spliceg_cond1=comb_spliceg_cond1)
                voila_output.write(st)
        ...
        with open(output_dir + "index.html", 'w') as voila_output:
            st = sum_template.render(lsvList=majiq_output['lsv_list'],
                                     tableMarks=table_marks_set(len(majiq_output['lsv_list'])),
                                     lexps=majiq_output['meta_exps'], links_dict=links_dict,
...
def parse_input(args):
    ...
    if type_summary == constants.COND_TABLE:
        ...
        render_summary(output_dir, output_html,
                       {'lsvs': lsvs_dict, 'sample_names': sample_names, 'cond_pair': cond_pair, 'thres': thres_change},
                       type_summary, logger=logger)
        return
